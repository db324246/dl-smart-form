...
import fieldRule from '@/components/dlSmartForm/reportForm/fieldRuleConfig'
...
export default {
  ...
  mutations: {
    ...
    // 保存布局节点
    setNode(state, { formId, key, value }) {
      const former = state.formerMap.get(formId)
      former.nodesMap.set(key, value)
    },
    // 保存拖拽中节点
    setDraggingNode(state, node) {
      state.draggingNode = deepClone(node)
    },
    // 设置正在聚焦的节点
    setFocusNodeKey(state, key) {
      state.focusNodeKey = key
    },
    // 保存字段关联规则
