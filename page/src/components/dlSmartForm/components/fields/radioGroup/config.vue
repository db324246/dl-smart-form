<template>
  <el-form class='radioGroup-config' label-position="top" size="small">
    ...
    <el-form-item label="选项" v-loading="loadingDict">
      <el-radio-group v-model="form.value" :disabled="disabled">
        <draggable
          tag="ul"
          :list="form.options"
...
export default {
  ...
  methods: {
    ...
    handleOptionsRemove(index, item) {
      ...
      this.form.value = this.form.value === item.key
        ? ''
        : this.form.value
    },
